...
func TestRunExcludeAllTags(t *testing.T) {
	...
	exitCode := run(runParams{
		...
		output:         "",
		outputPrefix:   "",
		includeSpecs:   []compiler.IncludeSpec{{Include: compiler.Exclude, TagGlob: "man*"}, {Include: compiler.Exclude, TagGlob: "ap*"}},
		verbose:        false,
		allowUpperCase: false,
		withReader:     mockReader(exampleInput),
		withWriter:     mockWriter(&outb),
...
func TestRunIncludeOnlySomeTags(t *testing.T) {
	...
	exitCode := run(runParams{
		...
		output:         "",
		outputPrefix:   "",
		includeSpecs:   []compiler.IncludeSpec{{Include: compiler.Include, TagGlob: "ap*"}},
		verbose:        false,
		allowUpperCase: false,
		withReader:     mockReader(exampleInput),
		withWriter:     mockWriter(&outb),
